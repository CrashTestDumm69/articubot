...
namespace hub_diffdrive
{
		...
		const rclcpp::Time & /*time*/, const rclcpp::Duration & period)
	{
		...
		int delta_count_l = p_l - wheel_l_.count;
		if(delta_count_l < -180)
			wheel_l_.rev++;
		else if(delta_count_l > 180)
			wheel_l_.rev--;
		wheel_l_.count = p_l;
		double delta_theta_l = (delta_count_l + wheel_l_.rev * 360) / (360 * 6.28319);
		wheel_l_.pos += delta_theta_l;

		int delta_count_r = p_r - wheel_r_.count;
		if(delta_count_r < -180)
			wheel_r_.rev++;
		else if(delta_count_r > 180)
			wheel_r_.rev--;
		wheel_r_.count = p_r;
		double delta_theta_r = (delta_count_r + wheel_r_.rev * 360) / (360 * 6.28319);
		wheel_r_.pos += delta_theta_r;

		std::cout << delta_theta_l << " " << delta_theta_r << std::endl;
