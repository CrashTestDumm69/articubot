...
namespace hub_diffdrive
{
		...
		const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
	{
		//int p_l, p_r;
		//double pos;
		comms_.read_motor_values(wheel_l_.vel, wheel_r_.vel, wheel_l_.pos, wheel_r_.pos);
		// wheel_l_.pos += (wheel_l_.vel * (period.seconds() / 60)) * 6.28319;
		// wheel_r_.pos += (wheel_r_.vel * (period.seconds() / 60)) * 6.28319;

		/*int delta_count_l = wheel_l_.count - p_l;
		if(delta_count_l <= -180)
			wheel_l_.rev--;
		else if(delta_count_l >= 180)
			wheel_l_.rev++;
		wheel_l_.count = p_l;
		pos = wheel_l_.rev * 360 + wheel_l_.count;
		wheel_l_.pos = (pos * M_PI) / 180;

		int delta_count_r = wheel_r_.count - p_r;
		if(delta_count_r <= -180)
			wheel_r_.rev--;
		else if(delta_count_r >= 180)
			wheel_r_.rev++;
		wheel_r_.count = p_r;
		pos = wheel_r_.rev * 360 + wheel_r_.count;
		wheel_r_.pos = (pos * M_PI) / 180;*/

		//std::cout << wheel_l_.count << " " << wheel_r_.count << std::endl;
	...
	hardware_interface::return_type hub_diffdrive ::HubDiffDriveHardware::write(
		const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
	{	
		comms_.set_motor_values(((wheel_l_.cmd * 60 * 7) / 44), ((wheel_r_.cmd * 60 * 7) / 44));
		return hardware_interface::return_type::OK;
	}
